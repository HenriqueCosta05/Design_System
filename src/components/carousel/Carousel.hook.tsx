...
const useCarousel = (props: CarouselProps) => {
    ...
    const getCarouselVariantName = useCallback(() => {
        switch (variant) {
            case "inside-control":
                return "glide-01";
            case "outside-control":
                return "glide-04";
            case "indicator-outside":
                return "glide-05";
            case "indicator-inside":
                return "glide-02";
            case "indicator-and-control-inside":
                return "glide-03";
            case "with-cards":
                return "glide-06";
            case "testimonials":
    ...
    useEffect(() => {
        ...
        const variantName = getCarouselVariantName();
        const selector = `.${variantName}`;
        glideInstance.current = new Glide(selector, options);

        setTotalSlides(items?.length || 0);

        if (onSlideChange) {
            glideInstance.current.on("move.after", () => {
                const newIndex = glideInstance.current?.index || 0;
    ...
    }, [variant, autoplay, autoplaySpeed, items, focusAt, gap, onSlideChange, getCarouselVariantName]);
    ...
    const renderSlideContent = useCallback((item: any, index: number) => {
    ...
    const getContainerClassName = useCallback(() => {
        const baseClassName = variant === 'with-cards'
            ? `${getCarouselVariantName()} relative overflow-hidden rounded bg-white shadow-xl shadow-slate-200`
            : `${getCarouselVariantName()} relative`;

        const customClass = classNames?.container || '';
        return `${baseClassName} ${customClass}`.trim();
    }, [variant, classNames, getCarouselVariantName]);


    const getContainerStyle = useCallback(() => {
        return {
            width: itemWidth || '100%',
            height: itemHeight || '400px', // Default height if not specified
            position: 'relative',
            overflow: 'hidden'
        } as const;
